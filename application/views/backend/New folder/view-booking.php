...
<div class="content-wrapper">
        ...
        <h1> &nbsp;</h1>
          <ol class="breadcrumb">
            ...
            <li><a href="<?php echo base_url(); ?>cadmin/listbookroom"> Danh sách </a></li>
  ...
  <?php if(isset($_GET['err']) && $_GET['err']!=null &&  $_GET['err']==1){ ?>
    ...
    <div class="box box-primary">
        ...
        <table id="list-cart" cellpadding="0" cellspacing="0" width="100%" border="0">
          ...
          <tr>
            <?php foreach ($this->madmin->get_data_where("tbl_hotel",array('id'=>$key['idhotel'])) as $k)
            ?>
            <td>#</td>
            <td><?php echo $k['title_vi']; ?></td>
            <td>
              <?php foreach ($this->madmin->get_data_where("tbl_room",array('id'=>$key['idroom'])) as $k)
                echo $k['title'];
              
            ?>
            </td>
            <td><?php echo number_format($k['price'],0,",","."); ?> VNĐ</td>
            <td><?php echo $key['numday']; ?></td>
            <td><?php echo $key['tu']; ?></td>
            <td><?php echo number_format($key['price'],0,",","."); ?> VNĐ</td>
          </tr>
          <tr>
            <td colspan="3" align="right">Tổng tiền : </td>
            <td colspan="4"> <b style="color: red"> <?php echo number_format($key['price'],0,",","."); ?></b> VNĐ</td>
          </tr>
        </table>
        </div>
      </div>     
